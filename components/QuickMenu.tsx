...
export type QuickAction = { label: string; id: string; href?: string; description?: string };
  ...
  return (
    <div className="wvb-chatbot-actions" aria-label={label}>
      {actions.map((action, index) => action.href ? (
        <a className="wvb-chatbot-action" key={`${action.id}-${index}`} href={action.href} target="_blank" rel="noopener noreferrer"><OptionIcon id={action.id} /><span>{action.label}{action.description && <small>{action.description}</small>}</span><i aria-hidden>›</i></a>
      ) : (
        <button className={`wvb-chatbot-action${action.id === 'back-topics' ? ' wvb-chatbot-action--back' : ''}`} type="button" key={`${action.id}-${index}`} onClick={() => onAction(action)}><OptionIcon id={action.id} /><span>{action.label}{action.description && <small>{action.description}</small>}</span><i aria-hidden>›</i></button>
      ))}
    </div>
  );
}
...
function iconFor(id: string): { src?: string; symbol: string } {
  ...
  if (id === 'machine-options') return { src: '/icon-rental.png', symbol: '' };
